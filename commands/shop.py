...
class ShopCommand:
    ...
    async def handle(self, event: AstrMessageEvent, action: str = "", item_id: str = "") -> None:
        ...
        if not action:
            event.set_result(MessageEventResult().message('❌ 请输入 "shop list" 查看商品列表'))
            return
        ...
        if action == "list":
            ...
            event.set_result(MessageEventResult().message('❌ 无效操作，请输入 "shop list" 或 "shop buy <商品ID>"'))
    ...
    async def _show_shop_list(self, event: AstrMessageEvent) -> None:
        ...
        for item in DEFAULT_SHOP_ITEMS:
            ...
            shop_list += f"类型: {'可存储物品' if item['storable'] else '立即生效道具'}\n\n"
        shop_list += '💡 提示: 输入 "shop buy <商品ID>" 购买商品'
        event.set_result(MessageEventResult().message(shop_list))

    async def _buy_item(self, event: AstrMessageEvent, user_id: str, platform: str, item_id: str) -> None:
        """购买物品"""
        if not item_id:
